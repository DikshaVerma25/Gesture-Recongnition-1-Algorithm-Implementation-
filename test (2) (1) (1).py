...
gestures = ["a", "b","c", "d", "e", "g", "k", "m", "n", "o", "p", "q", "r", "u", "w","x"]
...
# and generates a filename for the current gesture and count.    
    if len(points)>15:
        ...
        gesture_coords = []
        points=[]
#he points list is then cleared, and if the sum of all the gesture counts is equal to len(gestures) * 10, a "Thank you for drawing!" message is displayed on the canvas.
...
#checks if a folder named "user_gestures" exists in the current directory. If it doesn't, it creates on    
    current_dir=os.getcwd()
    folder_path = current_dir+"/user_gestures"
