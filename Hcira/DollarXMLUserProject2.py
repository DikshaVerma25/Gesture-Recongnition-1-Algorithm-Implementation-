...
class Gesture:
    ...
    def __init__(self, pointsd, flag=True):
        ...
        if flag:
            ...
            self.vectorpoints=self.vectorizeTo()
    
    def vectorizeTo(self):
        sum = 0.0
        vector = []
        for i in range(len(self.pointse)):
            vector.append(self.pointse[i][0])
            vector.append(self.pointse[i][1])
            sum += (self.pointse[i][0] * self.pointse[i][0]) + (self.pointse[i][1] * self.pointse[i][1])
        ...
        for user in range(numUsers):
            ...
            for gesture in range(numGestures):
                ...
                for t in range(numForEachGesture):
                    ...
                    arr=Gesture(d)
                    data[user][1][gesture][1][t]=arr.vectorpoints
                    if len(arr.vectorpoints)==128:
                        c=c+1
                    else:
                        cn=cn+1
        #def write_data_to_csv(csv_file_path, data):
  ...
  # recognition gesture function will perform golden search using the golden ratio which calls the method distance at best angle
    def rec_ges(self, points,Templates2):
        ...
        for template_stroke in copyTemplate:
            ...
            print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&",len(template_stroke[3]),len(points))
            d=ges.optimalcosinedistance(template_stroke[3],points)
